...
#这段代码实现了基于特征点匹配的相机位姿估计；
def estimate_pose(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    if len(kpts0) < 5:
        return None
    #  # 如果特征点数量小于5个，无法进行姿态估计，因此返回None

    # normalize keypoints
    kpts0 = (kpts0 - K0[[0, 1], [2, 2]][None]) / K0[[0, 1], [0, 1]][None]
    kpts1 = (kpts1 - K1[[0, 1], [2, 2]][None]) / K1[[0, 1], [0, 1]][None]
    ## 对特征点坐标进行归一化处理，使用相机内参矩阵进行归一化

    # normalize ransac threshold
    ransac_thr = thresh / np.mean([K0[0, 0], K1[1, 1], K0[0, 0], K1[1, 1]])
    # # 对 RANSAC 阈值进行归一化处理，使用相机内参平均值进行归一化
    ...
    if E is None:
        ...
        return None
    #使用cv2计算本质矩阵E，并利用RANSAC算法进行求解，得到内点mask；threshold 是 RANSAC 阈值，prob 是 RANSAC 的置信度参数，method 指定使用 RANSAC 方法。
    ...
    for _E in np.split(E, len(E) / 3):
        ...
        if n > best_num_inliers:
            ...
            best_num_inliers = n
    return ret
## 从基础矩阵E中恢复姿态，对每个可能的解进行评估，并选择内点最多的解；
# 这段代码通过特征点匹配计算相机的本质矩阵，从而估计相机的姿态。
...
def aggregate_metrics(metrics, epi_err_thr=5e-4):
    ...
    # aucs = error_auc("Rt", pose_errors, angular_thresholds)  # (auc@5, auc@10, auc@20)
    ...
    # return {**aucs, **precs}
    # return {**aucs, **rotation_aucs, **translation_aucs}
